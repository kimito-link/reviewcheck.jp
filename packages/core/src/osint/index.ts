...
/**
 * Pointer Map: 活動ネーム（インフルエンサー・配信者・作家等）向けカタログ。
 *
 * ★対象は「活動ネーム＝公に名乗っている職業上の名前」であって、
 *   特定個人の所在・行動・素行ではない（設計 §4-4 探偵業法の線を越えない）。
 *   本名・住所・勤務先を調べる用途には使わせない。targetType は "brand" を使う。
 *
 * ★★件数を出さない理由（2026-08-19 実測）:
 *   匿名掲示板の言及数は**サーバーから正しく取得できない**。
 *     - 掲示板を直接叩く → robots.txt で ClaudeBot 等を名指し拒否・素の取得は 403
 *     - Bing の site: 件数 → **数字が信用できない**。実在しない名前で 116,000 件、
 *       別ドメインと同一値が返った（既存 google-count のコメントにも
 *       「Bing は site: 件数を出さない」と明記されている）
 *     - DuckDuckGo → CAPTCHA で遮断
 *   → **「調べていないのに数字を出す」ことになるので件数は持たない。**
